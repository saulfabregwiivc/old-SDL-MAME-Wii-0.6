...
static const options_entry mame_sdl_options[] =
{
	...
	{ SDLOPTVAL_SAFEAREA "(0.01-1)",          "1.0",      0,                 "Adjust video for safe areas on older TVs (.9 or .85 are usually good values)" },
...
static void log_errors(void *param, const char *format, va_list argptr)
{
	char buffer[1024];
	char buffer2[1152];
	char ctime[128];
	...
	{
		strftime(ctime, sizeof(ctime), "%F %T: ", tmp);
	}

	vsnprintf(buffer, ARRAY_LENGTH(buffer), format, argptr);
	sprintf(buffer2, "%s%s", ctime, buffer);

	errlog = fopen("errors.txt", "a");

	if (errlog != NULL)
	{
		fputs(buffer2, errlog);
		fclose(errlog);
	}
}
...
{
	int res = 0;
	// Is it SendElf that's passing bad parameters? To lazy to test
	const char *nargv[] = {
		""
	};
	int nargc = 1;
