...
static void sdlinput_register_mice(running_machine *machine)
{
	...
	for (wiimote = 0; wiimote < 4; wiimote++)
	{
		char *mouse_name = remove_spaces(machine, SDL_JoystickName(wiimote));
		
		devmap_register(&mouse_map, wiimote, mouse_name);
	}

	mame_printf_verbose("Wiimote: Start initialization\n");
	for (index = 0; index < 4; index++)
	{
		device_info *devinfo;
		char defname[90];

		devinfo = devmap_class_register(machine, &mouse_map, index, &mouse_list, DEVICE_CLASS_LIGHTGUN);
		...
		mame_printf_verbose("Wiimote: Registered %s\n", devinfo->name);
	}
	mame_printf_verbose("Wiimote: End initialization\n");
}
...
static void sdlinput_register_mice(running_machine *machine)
{
	...
	for (index = 0; index < MAX_DEVMAP_ENTRIES; index++)
	{
		...
		if (0 && mouse_enabled)
			SDL_SetRelativeMouseMode(index, 1);
...
void sdlinput_poll(running_machine *machine)
{
	...
	for (index = 0; index < 4; index++)
	{
		devinfo = generic_device_find_index(mouse_list, mouse_map.logical[index]);
		WPAD_IR(index, &ir);
		...
		else
		{
			...
			devinfo->mouse.lX = INPUT_ABSOLUTE_MAX+1;
			devinfo->mouse.lY = INPUT_ABSOLUTE_MAX+1;
		}
	}
#endif
#if SDL13_COMBINE_RESIZE
